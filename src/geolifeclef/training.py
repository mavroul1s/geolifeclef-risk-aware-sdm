...
def train_temporal(model: nn.Module, train_loader: DataLoader, val_loader: DataLoader, device: torch.device, modality: str, epochs: int, lr: float, weight_decay: float, mixed_precision: bool, output_dir: Path, patience: int = 6) -> dict[str, Any]:
    optimizer = torch.optim.AdamW(model.parameters(), lr=lr, weight_decay=weight_decay); criterion = nn.BCEWithLogitsLoss(); scaler = torch.amp.GradScaler("cuda", enabled=mixed_precision and device.type == "cuda")
    if device.type == "cuda":
    ...
    for epoch in range(1, epochs + 1):
        ...
        elapsed = time.perf_counter() - started; targets, probabilities = predict(model, val_loader, device, modality); score = multilabel_f1(targets, probabilities)
        record = {"epoch": epoch, "train_loss": total_loss / max(samples, 1), "val_micro_f1": score, "seconds": elapsed, "examples_per_second": samples / max(elapsed, 1e-6)}; history.append(record)
        if score > best_f1:
    ...
    return {"best_val_micro_f1": best_f1, "epochs_completed": len(history), "last_epoch": history[-1], "peak_gpu_memory_mb": torch.cuda.max_memory_allocated(device) / 1024**2 if device.type == "cuda" else None}
