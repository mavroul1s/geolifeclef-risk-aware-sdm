"""Build the compact, self-contained GeoLifeCLEF v27 Kaggle notebook."""
...
EXPECTED_V26_HASH = "67937cf92f0b4626b7b3c643ababbd0c7a4f39801909e4bd5646b532371c21a9"
...
def canonical_submission_bytes(path: Path) -> bytes:
    ...
    if hashlib.sha256(values).hexdigest() == EXPECTED_V26_HASH:
        return values
    for suffix in (b"\r\n", b"\n"):
        if hashlib.sha256(values + suffix).hexdigest() == EXPECTED_V26_HASH:
            return values + suffix
    raise ValueError("The supplied v26 submission does not match the frozen official artifact")


def packed_v26_submission(submission_path: Path, species_path: Path) -> tuple[str, str, str]:
    ...
    if list(frame.columns) != ["surveyId", "predictions"] or len(frame) != EXPECTED_ROWS:
        raise ValueError("Unexpected v26 submission schema")
    ...
    for text in frame.predictions.astype(str):
        ...
        if (columns < 0).any() or len(columns) != len(np.unique(columns)) or not 10 <= len(columns) <= 40:
            raise ValueError("Invalid v26 prediction row")
...
def make_notebook(core: str, control_b64: str, control_sha256: str, control_raw_sha256: str,
    ...
    markdown = """# GeoLifeCLEF 2025 — v27 multi-scale shift-aware ensemble
...
"""
    payload_cell = (
        ...
        f"V27_SOURCE_COMMIT = {source_commit!r}\n"
        f"FROZEN_V26_PAYLOAD_SHA256 = {control_sha256!r}\n"
        f"FROZEN_V26_RAW_SHA256 = {control_raw_sha256!r}\n"
        f"FROZEN_V26_PAYLOAD_B64 = {control_b64!r}\n"
        ...
        "print({'embedded_v26_bytes': len(FROZEN_V26_PAYLOAD_B64), "
    ...
    return {
        "cells": [
            {"cell_type": "markdown", "id": "v27-intro", "metadata": {},
             "source": source_lines(markdown)},
            {"cell_type": "code", "id": "v27-core", "execution_count": None,
             "metadata": {}, "outputs": [], "source": source_lines(core)},
            {"cell_type": "code", "id": "v27-evidence", "execution_count": None,
             "metadata": {}, "outputs": [], "source": source_lines(payload_cell)},
            {"cell_type": "code", "id": "v27-run", "execution_count": None,
        ...
        "metadata": {
            ...
            "glc_v27": {
                ...
                "frozen_v26_submission_sha256": EXPECTED_V26_HASH,
                ...
                "final_output": "/kaggle/working/v27_export/GLC25_PA_submission_v27.csv",
...
def main() -> None:
    ...
    parser.add_argument("--v26-output", type=Path,
                        default=repository / "results/v26_kaggle_output")
    parser.add_argument("--species", type=Path,
                        default=repository / "artifacts/v20_frozen_bundle_v21/species_ids.npy")
    parser.add_argument("--output", type=Path,
                        default=repository / "notebooks/geolifeclef_v27_multiscale_shift_aware_ensemble.ipynb")
    ...
    consumed_paths = [
        ...
        args.v26_output / "assessment_per_survey_v26.csv",
    ]
    control = packed_v26_submission(args.v26_output / "GLC25_PA_submission_v26.csv", args.species)
    consumed = packed_consumed_ids(consumed_paths)
    core_path = repository / "scripts/v27_notebook_core.py"
