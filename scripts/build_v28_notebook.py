"""Build the compact, self-contained GeoLifeCLEF v28 Kaggle notebook."""
...
EXPECTED_V27_HASH = "32cd02788d910abe0cd18715da00f201a52a371a2135d17399301dd7e532f710"
...
def canonical_submission_bytes(path: Path) -> bytes:
    ...
    if hashlib.sha256(values).hexdigest() == EXPECTED_V27_HASH:
        return values
    for suffix in (b"\r\n", b"\n"):
        if hashlib.sha256(values + suffix).hexdigest() == EXPECTED_V27_HASH:
            return values + suffix
    raise ValueError("The supplied v27 submission does not match the frozen official artifact")


def packed_v27_submission(submission_path: Path, species_path: Path) -> tuple[str, str, str]:
    ...
    if list(frame.columns) != ["surveyId", "predictions"] or len(frame) != EXPECTED_ROWS:
        raise ValueError("Unexpected v27 submission schema")
    ...
    for text in frame.predictions.astype(str):
        ...
        if (columns < 0).any() or len(columns) != len(np.unique(columns)) or not 10 <= len(columns) <= 40:
            raise ValueError("Invalid v27 prediction row")
...
def make_notebook(core: str, control_b64: str, control_sha256: str, control_raw_sha256: str,
    ...
    markdown = """# GeoLifeCLEF 2025 — v28 presence-only shift mixture-of-experts
...
"""
    payload_cell = (
        ...
        f"V28_SOURCE_COMMIT = {source_commit!r}\n"
        f"FROZEN_V27_PAYLOAD_SHA256 = {control_sha256!r}\n"
        f"FROZEN_V27_RAW_SHA256 = {control_raw_sha256!r}\n"
        f"FROZEN_V27_PAYLOAD_B64 = {control_b64!r}\n"
        ...
        "print({'embedded_v27_bytes': len(FROZEN_V27_PAYLOAD_B64), "
    ...
    return {
        "cells": [
            {"cell_type": "markdown", "id": "v28-intro", "metadata": {},
             "source": source_lines(markdown)},
            {"cell_type": "code", "id": "v28-core", "execution_count": None,
             "metadata": {}, "outputs": [], "source": source_lines(core)},
            {"cell_type": "code", "id": "v28-evidence", "execution_count": None,
             "metadata": {}, "outputs": [], "source": source_lines(payload_cell)},
            {"cell_type": "code", "id": "v28-run", "execution_count": None,
        ...
        "metadata": {
            ...
            "glc_v28": {
                ...
                "frozen_v27_submission_sha256": EXPECTED_V27_HASH,
                ...
                "final_output": "/kaggle/working/v28_export/GLC25_PA_submission_v28.csv",
...
def main() -> None:
    ...
    parser.add_argument("--v27-output", type=Path,
                        default=repository / "results/v27_kaggle_output")
    parser.add_argument("--species", type=Path,
                        default=repository / "artifacts/v20_frozen_bundle_v21/species_ids.npy")
    parser.add_argument("--output", type=Path,
                        default=repository / "notebooks/geolifeclef_v28_presence_only_shift_moe.ipynb")
    ...
    consumed_paths = [
        ...
        repository / "results/v25_kaggle_output/assessment_per_survey_v25.csv",
        repository / "results/v26_kaggle_output/assessment_per_survey_v26.csv",
        args.v27_output / "assessment_per_survey_v27.csv",
    ]
    control = packed_v27_submission(args.v27_output / "GLC25_PA_submission_v27.csv", args.species)
    consumed = packed_consumed_ids(consumed_paths)
    core_path = repository / "scripts/v28_notebook_core.py"
