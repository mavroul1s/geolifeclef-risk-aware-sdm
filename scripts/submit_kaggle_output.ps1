...
try {
    ...
    $submissionEntry = $outputs.files | Where-Object {
        $_.fileName -eq "geolifeclef-risk-aware-sdm/artifacts/official_pa_submission/GLC25_PA_submission.csv"
    } | Select-Object -First 1
    $manifestEntry = $outputs.files | Where-Object {
        $_.fileName -eq "geolifeclef-risk-aware-sdm/artifacts/official_pa_submission/submission_manifest.json"
    } | Select-Object -First 1
    ...
    if ([int]$manifest.submission.prediction_policy.k -ne 18) {
