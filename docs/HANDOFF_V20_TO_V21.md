...
Original v21 artifacts have been selectively preserved under ignored `artifacts/v21_frozen_source/`. The compact input `artifacts/v21_frozen_bundle/` passed exact original-rank probability parity; the original CSV is preserved. Local tests/docs were completed and committed (`1ba5cae`). **The user explicitly approved the private 223.7 MB transfer to `con1los/geolifeclef-v21-frozen-control`; do not ask again.** The approved upload is in progress. Earlier approval rejections were resolved by this explicit consent. The payload contains competition-derived probabilities/policies/report/submission only, no credentials or external data. Confirm dataset readiness and attach version `/1` alongside the existing v20 bundle before pushing. No v22 kernel push has occurred yet, so version 21 remains the latest output.
