...
Use a small, predetermined set of conservative mixture strengths and OOD-support gates, including exactly zero expert weight. Keep the original-v20 top-20 output policy available unchanged. Choose policies solely from the appropriate calibration partition, with deterministic tie-breaking that prefers less deviation from v20. Persist the full candidate list and selected policies before assessment. Evaluation and production calibration may select different policies, as they use separately fitted compatible predictors; report the difference transparently.
...
Make at most **one official submission**, only if the already-frozen PO candidate has positive sample-F1 gain over the compatible frozen-v20 control on the new assessment and all source, split, artifact, output and runtime checks pass. Also report whether it beats zero-PO; do not claim demonstrated PO benefit otherwise. If the PO component is zero, the assessment is invalid, any integrity check fails, or the candidate fails to beat the control, save the outcome and do not submit. Do not make a second attempt based on the resulting leaderboard score.
